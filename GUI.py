...
class SegmentViewer(QMainWindow):
    def __init__(self, window_size_sec=10.0, sampling_rate=125):
        ...
        self.xdf_poll_timer = QTimer()
        self.xdf_poll_timer.timeout.connect(self._poll_for_xdf)
    ...
    def _poll_for_xdf(self):
        """Drain BrainFlow board data into the XDF recorder.

        Runs on the xdf_poll_timer (50 ms) only when visualization is NOT
        active; when visualization IS active, update_stream_data() already
        drains the board and also pushes to the recorder.
        """
        if self.board is None or self.xdf_recorder is None or not BRAINFLOW_AVAILABLE:
            return
        if self.streaming_active:
            return  # update_stream_data() handles the board drain
        try:
    ...
    def start_xdf_recording(self, patient_id):
        ...
        self.xdf_poll_timer.start(50)  # 20 Hz fallback when viz is off

    def stop_xdf_recording(self):
        """Finalise and save the XDF file."""
        self.xdf_poll_timer.stop()
        if self.xdf_recorder is not None:
